...
const CalculatorWizard: React.FC<Props> = ({ data, setData }) => {
  ...
  const fetchAttom = async (address1: string, address2: string) => {
    ...
    try {
      ...
      const attomData = json;
      
      // Update data with normalized ATTOM results
      setData((f) => ({
        ...f,
        county: attomData.county || f.county,
        parcelId: attomData.apn || f.parcelId,
        taxValue: useTaxEstimate ? (attomData.tax?.annual ?? f.taxValue) : f.taxValue,
        taxYear: attomData.tax?.year ?? f.taxYear,
        assessedTotal: attomData.assessed?.total ?? f.assessedTotal,
      }));
      
      return json;
    } catch (err) {
      setAttomStatus("error");
      setAttomError("Property data lookup currently unavailable.");
      return null;
    }
  };
      ...
      try {
        ...
        autocompleteRef.current.addListener('place_changed', async () => {
          ...
          if (place && place.address_components) {
            ...
            if (state && state !== 'OH') {
              setAddressError('Out of State: This tool currently only supports Ohio properties. Please contact info@worldclasstitle.com or call us at (614) 848-4000 for assistance with this transaction.');
        ...
        console.warn('Google Maps Autocomplete failed to initialize. Likely restricted API key.');
      }
    }
  }, [step, setData]);
    ...
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <div className="bg-white p-8 rounded-[40px] shadow-2xl shadow-slate-200/50 border border-slate-100 min-h-[450px] flex flex-col">
        ...
        <div className="flex-grow">
          {step === 1 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <h2 className="text-2xl font-display font-bold text-brand-primary uppercase tracking-tight">Property Address</h2>
                <div className="flex flex-wrap gap-2">
                   <span className={`px-2 py-1 rounded-[6px] text-[9px] font-bold uppercase tracking-wider ${(window as any).google?.maps ? 'bg-green-50 text-green-600 border border-green-100' : 'bg-slate-50 text-slate-400 border border-slate-100'}`}>Google Places: {(window as any).google?.maps ? 'Connected' : 'Error'}</span>
              ...
              <div className="grid grid-cols-1 gap-4">
                <div className="relative">
                  <div className="flex justify-between items-end mb-1">
                    <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest">Property Search</label>
                    {data.address && (
                  ...
                  {addressError && (
                    <div className="mt-3 p-4 bg-red-50 border-l-4 border-red-500 text-red-700 text-xs font-bold rounded-r-xl animate-in slide-in-from-top-1">
                      {addressError}
                    </div>
                  )}
                  {attomStatus === 'loading' && (
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 uppercase mb-1 tracking-widest">Parcel / APN</label>
                    <input 
                      type="text" 
                ...
                <div className="flex justify-between items-center py-2">
                  <div className="flex items-center gap-1">
                    {attomStatus === 'success' && (
                       <span className="text-[10px] font-bold text-green-600 uppercase bg-green-50 px-2 py-0.5 rounded border border-green-100 flex items-center shadow-sm">
                ...
                <div className="pt-4 p-6 bg-slate-50 rounded-[32px] border border-slate-100">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">Set your primary target price</p>
                    </div>
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-400 uppercase mb-1 tracking-widest">Closing Date</label>
                  <input 
                    type="date" 
          ...
          {step === 2 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex items-center justify-between p-6 bg-slate-50 rounded-[32px] mb-4 border border-slate-100">
                <span className="font-bold text-slate-700">Existing Mortgages?</span>
                <button 
                  ...
                  {data.mortgagePayoffs.map((payoff, idx) => (
                    <div key={payoff.id} className="p-6 border border-slate-100 rounded-[32px] bg-white shadow-sm space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1">
                           <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Current Balance</label>
                           <input 
                             type="number" 
          ...
          {step === 4 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <h2 className="text-2xl font-display font-bold text-brand-primary uppercase tracking-tight">Credits & Concessions</h2>
              <div className="grid grid-cols-1 gap-4">
                <div className="bg-slate-50 p-6 rounded-[32px] border border-slate-100">
                  <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-3">Seller Credit to Buyer ($)</label>
                  <input 
                    type="number" 
          ...
          {step === 5 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="p-6 bg-slate-50 rounded-[40px] border border-slate-100">
                <h3 className="text-[11px] font-bold text-brand-primary uppercase mb-5 tracking-widest">Title Insurance Config</h3>
                <div className="space-y-5">
                  <div className="flex items-center justify-between">
                    <div className="max-w-[70%]">
                      <span className="text-sm font-bold text-slate-700">Homeowner's Policy (PR-1.1)</span>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">Premium coverage with 15% rate multiplier</p>
                    </div>
                    <button 
                  ...
                  <div className="flex items-center justify-between pt-5 border-t border-slate-200">
                    <div className="max-w-[70%]">
                      ...
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">30% discount if prior policy is available</p>
                    </div>
                    <button 
              ...
              <div className="p-8 bg-brand-light/10 rounded-[40px] border border-brand-light/30">
                ...
                <div className="relative">
                  ...
                  />
                  {data.taxYear && (
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-1 rounded border uppercase">Tax Year: {data.taxYear}</span>
                  )}
                </div>
                <div className="mt-4 flex flex-col gap-1">
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tight flex items-center">
                    ...
                    Taxes will be prorated through {new Date(data.closingDate).toLocaleDateString()}.
                  </p>
                  {attomStatus === 'success' && (
                     <p className="text-[9px] text-green-600 font-bold uppercase">Verified via ATTOM database</p>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-50 p-5 rounded-[32px] border border-slate-100">
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1 tracking-widest">HOA Transfer Fee</label>
      ...
      <p className="mt-8 text-center text-[10px] text-slate-400 font-bold uppercase tracking-widest">
        Property Data lookup provided by <span className="text-brand-primary">ATTOM</span> & <span className="text-brand-primary">Google Places</span>
      </p>
    </div>
  );
};
