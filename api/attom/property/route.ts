  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return NextResponse.json(
        { error: "ATTOM request failed", status: resp.status, details: text.slice(0, 300) },
        { status: 502 }
      );
    }
    ...
    const apn =
      p?.identifier?.apn ||
      p?.identifier?.apnOrig ||
      p?.parcel?.apn ||
      null;

    const county =
      p?.area?.countyName ||
      p?.area?.countrySecSubdName ||
      null;

    const taxYear =
      p?.assessment?.tax?.taxYear ||
      p?.tax?.taxYear ||
      null;

    const taxAmt =
      p?.assessment?.tax?.taxAmt ||
      p?.tax?.taxAmt ||
      null;
